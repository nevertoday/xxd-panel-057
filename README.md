...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092160522711978218"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 057 样张 1"></a></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 057 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 057 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 057 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 057 样张 8"></td>
  </tr>
</table>

## 新增 3:4 上下双联样张

以下四张复用上方 16:9 样张对应的四张独立素材，重新生成完整 3:4 上下双联画布；上部保留现实摄影，下部遵循 057 原始提示词重构。英文配字只从当前照片的内容、情绪或隐喻中生成，并已逐张复核。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 057 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 057 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 057 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 057 新增上下样张 4"></td>
  </tr>
</table>

> [小小东（@xiaoxiaodong01）](https://x.com/xiaoxiaodong01/status/2092160522711978218) · 2026 年 8 月 25 日<br>
> GPT2 × 几何 × 拼贴治愈系 × 美学提示词 × VOL.057

<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2092160522711978218">查看原推文与完整提示词 →</a></p>

这些样张只展示 057 的审美结果；样张主体、构图、配色、文字和旧画幅不会成为新任务的参考或默认值。

几何构成 · 智能马赛克 · 建筑图解 · 艺术地图 · 暖冷色块

从照片里提取最值得记住的主体、轮廓与关系，再将它们压缩为几何块面、矩形单元、细线框架和层叠网格。近白留白与温暖明快的冷暖色块，让成品兼具建筑图解的理性和记忆的温度。
...
**Panel 057** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**用几何构成与智能马赛克，把照片重组为安静而温暖的现代拼贴**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
> 用 XXD Panel 057 帮我处理这张图，先推荐最合适的构图和尺寸。
...
## 快速判断：XXD Panel 057 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 用几何构成与智能马赛克，把照片重组为安静而温暖的现代拼贴 |
| **一眼可辨的特点** | 几何构成 · 智能马赛克 · 建筑图解 · 艺术地图 · 暖冷色块 |
...
这套风格先判断照片真正成立的主体、轮廓、姿态和叙事关系，再删去无关环境，把值得记住的信息重组为清楚的几何块面与模块秩序。它不是随机像素化，也不把所有物体逐一插画化。

```text
锁定身份、轮廓与关键关系 → 删除无关环境与细节 → 提炼最值得记住的信息 → 压缩为几何块面、矩形单元与层叠网格 → 以近白留白建立空气和距离 → 用暖冷色块、轴线与对齐秩序完成智能马赛克重组 → 融入极少且有意义的文字
```

## 成品中最容易识别的风格特点

- 大形体仍保持清楚轮廓，小细节转译为有秩序的色块、格点、切片、台阶边缘与局部拼接。
- 矩形单元、细线框架、层叠网格、局部轴线和对齐线共同形成建筑化秩序，而不是随机像素噪声。
- 极浅、近白并随源图综合色温变化的背景，让几何主体在大面积留白中保持轻、净、安静。
- 奶油黄、杏色、粉橘、珊瑚粉等暖色与天空蓝、浅青绿、薄荷绿和灰绿形成松弛的冷暖碰撞。
- 主体可偏置、悬置、低重心或向中心聚合；细瘦微排版进入留白或几何分区，不机械套用标题模板。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-057-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 057 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-057 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-057 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-057 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 057；公共设置只解析一次
/xxd-panel-057 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-057`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-057-prompt.en.md) · [中文运行适配器](references/xxd-panel-057-prompt.zh-CN.md)
